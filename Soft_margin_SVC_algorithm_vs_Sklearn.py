...
if False:
    ...
    # Crear y entrenar el modelo SVM DE SKLEARN!!!
    svm = LinearSVC()
    svm.fit(X,y)
    ...
    ylim = ax.get_ylim()
    xx, yy = np.meshgrid(np.linspace(xlim[0], xlim[1], 30), np.linspace(ylim[0], ylim[1], 30))
    zz = lambda x,y: (-svm.intercept_[0]-svm.coef_[0][0]*x -svm.coef_[0][1]*y)/svm.coef_[0][2]


    # Dibujar el hiperplano de decisión
    
    ax.plot_surface(xx, yy, zz(xx,yy), color='green', alpha=0.3)  # Cambiamos el color del hiperplano a verde
    ax.set_xlabel('X1')
    ax.set_ylabel('X2')
    ax.set_zlabel('X3')
    plt.show()
